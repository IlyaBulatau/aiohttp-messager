...
from src.database.connect import close_connect_with_database, connect_with_database
from src.database.models import create_tablse
from src.logger.setup import setup_loggger
from src.utils.context_processors import (
    context_get_username_processor,
    context_user_is_anonymous_processor,
)
from src.utils.decorators import add_user_id_to_request_middleware
from src.utils.user_sessions import AuthPolicy, setup_redis
...
def setup_templates(app: web.Application) -> None:
    path_to_templates = (
        pathlib.Path().absolute().joinpath("src").joinpath("app").joinpath("templates")
    )
    aiohttp_jinja2.setup(
        app,
        loader=jinja2.FileSystemLoader(searchpath=path_to_templates),
        context_processors=[
            context_user_is_anonymous_processor,
            context_get_username_processor,
...
def setup_app(app: web.Application) -> None:
    ...
    setup_security(
        app, identity_policy=SessionIdentityPolicy(), autz_policy=AuthPolicy()
    )
