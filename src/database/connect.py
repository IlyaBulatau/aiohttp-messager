...
async def connect_with_database(app: web.Application) -> None:
    ...
    """
    connect: asyncpg.Connection = await asyncpg.connect(host=app['config']['POSTGRES_HOST'],
    ...
    new_connect: asyncpg.Connection = await asyncpg.connect(host=app['config']['POSTGRES_HOST'],
                                    user=app['config']['POSTGRES_LOGIN'],
                                    password=app['config']['POSTGRES_PASSWORD'],
...
async def check_if_database_is_exist(connect: asyncpg.Connection, database_name: str) -> bool:
    """
